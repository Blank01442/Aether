...
ASTNode* factor() {
    ...
    if (t.type == TOKEN_NUMBER) {
        ...
        if (strcmp(t.value, "exit") == 0) {
            ASTNode* n = create_node(NODE_EXIT, t.line); return n;
        }
        if (strcmp(t.value, "screen_width") == 0 || strcmp(t.value, "screen_height") == 0) {
            if (peek().type == TOKEN_LPAREN) { consume(); match(TOKEN_RPAREN, ")"); }
            ASTNode* n = create_node(NODE_SCREEN_INFO, t.line); n->data.var_name = strdup(t.value); n->result_type = TYPE_INT; return n;
        }
